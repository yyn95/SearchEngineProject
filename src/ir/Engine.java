    ...
    public Engine( String[] args ) {
        ...
        switch(indexType){
            case "HashedIndex":
                ...
                break;
            case "PersistentHashedIndex":
                index = new PersistentHashedIndex();
                break;
            default:
                System.out.println("Unable to detect index type, set to HashedIndex");
                index = new HashedIndex();
        }

        // Initialize other instances
        kgIndex = new KGramIndex(2); // set a bi-gram index
        ...
        if (is_indexing) {
            synchronized ( indexLock ) {
                ...
                index.cleanup();
                //let searcher to build docIDs, load pagerank file, and initialize HITS
                searcher.buildNameToID();
                searcher.BuildPageRank();
                searcher.hr = new HITSRanker( "HITS/linksDavis.txt", "HITS/davisTitles.txt", null);
            }
        } else {
